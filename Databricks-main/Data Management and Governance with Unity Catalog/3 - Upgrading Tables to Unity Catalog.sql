...
SELECT * 
FROM IDENTIFIER('hive_metastore.' || 'mafalda_rufino' || '.movies')
LIMIT 10
...
DESCRIBE EXTENDED IDENTIFIER('hive_metastore.' || 'mafalda_rufino' || '.movies')
...
SELECT * FROM IDENTIFIER('hive_metastore.' || 'mafalda_rufino' || '.movies');
...
-- MAGIC %python
-- MAGIC df = spark.sql("select * from hive_metastore.mafalda_rufino.movies ")
-- MAGIC # print(df.columns)
-- MAGIC # print(df.dtypes)
-- MAGIC # print(df.schema)
-- MAGIC # print(df.describe())
-- MAGIC # print(df.printSchema())
-- MAGIC # print(df.show())
-- MAGIC # print(df.count())
-- MAGIC df.toPandas().describe()
-- MAGIC
-- MAGIC # temp view , global temp view, local temp view #
-- MAGIC #Session level temp view
-- MAGIC # df.createOrReplaceTempView("movies")
-- MAGIC # df.createOrReplaceGlobalTempView("movies")    

...
INSERT INTO finance.hone
SELECT
  id,
  current_date() - CAST(rand() * 30 AS INT) AS transaction_date,
  CAST(rand() * 10000 AS DECIMAL(18,2)) AS amount,
  concat('Transaction ', id) AS description,
  CAST(rand() * 1000 AS INT) AS account_id
FROM (
  SELECT posexplode(sequence(1, 100)) AS (id, _)
) t;
...
-- MAGIC %python
-- MAGIC spark.sql(f"REVOKE USE CATALOG,USE SCHEMA,SELECT ON CATALOG {DA.catalog_name} FROM `account users`").show()
...
SELECT * 
FROM databrickstheloop.finance.hone
...
CREATE OR REPLACE FUNCTION mrn_mask(mrn STRING)
  RETURN CASE WHEN NOT is_member('Financial Analyst') 
    THEN mrn 
    ELSE 'REDACTED' 
  END;
...
SELECT
  CASE WHEN
    is_account_group_member('HR Analyst') THEN mrn 
    ELSE 'REDACTED'
  END AS mrn,
  CASE WHEN

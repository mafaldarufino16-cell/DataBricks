...
-- MAGIC
-- MAGIC    b. Locate the catalog called **databrickstheloop** and expand the catalog. 
-- MAGIC
...
-- MAGIC %md
-- MAGIC 3. We want to modify our default catalog and default schema to use **databrickstheloop** and our **labuser** schema to avoid writing the three-level namespace every time we query and create tables in this course.
-- MAGIC
...
-- MAGIC
-- MAGIC     Run the following cell to set and view your default catalog and schema. Confirm that your default catalog is **databrickstheloop** and your schema is **labuser** (this uses the `DA.schema_name` variable created in the classroom setup script).
-- MAGIC
...
-- MAGIC ### B1. Catalogs
-- MAGIC
-- MAGIC Use the `SHOW SCHEMAS IN` statement to view available schemas in the **databrickstheloop** catalog. Run the cell and view the results. Notice that your **labuser** schema is within the **databrickstheloop** catalog.
...
-- MAGIC %md
-- MAGIC ### B2. Schemas
-- MAGIC Run the `DESCRIBE SCHEMA EXTENDED` statement to see information about your **labuser** schema (database) that was created for you within the **databrickstheloop** catalog. In the output below, your schema name is in the row called *Namespace Name*.  
-- MAGIC
...
DESCRIBE SCHEMA EXTENDED neha_priya;

...
# Lista das colunas JSON a converter
json_cols = ["tags", "recent_orders", "browsing_history"]

# Dicionário para guardar schemas inferidos
schemas = {}

# Inferir schema e converter JSON para StructType
events_struct_df = events_df

for c in json_cols:
    # Pega numa amostra não nula
    sample_json = events_df.select(c).filter(col(c).isNotNull()).limit(1).collect()[0][0]
    
    # Inferir schema da coluna JSON
    schemas[c] = schema_of_json(sample_json)
    print(f"Schema inferido para a coluna {c}: {schemas[c]}")
    
    # Converter a coluna JSON para tipo estruturado
    events_struct_df = events_struct_df.withColumn(c, from_json(col(c), schemas[c]))

# Mostrar schema final e amostra dos dados convertidos
events_struct_df.printSchema()
display(events_struct_df)
...
from pyspark.sql.functions import col, explode, size, count, desc
from pyspark.sql.types import ArrayType, StructType, StructField, StringType, DoubleType, TimestampType

# 1️⃣ Define schemas corretos caso ainda não estejam
tags_schema = ArrayType(StringType())  # se tags for apenas lista de strings

recent_orders_schema = ArrayType(
    StructType([
...
browsing_history_schema = ArrayType(
    StructType([
...
# 2️⃣ Converter JSON strings em tipos estruturados
parsed_df = ecommerce_df.select(
    "customer_id",
    "name",
    "email",
...
# 3️⃣ Contar número de tags e histórico por cliente
customer_counts_df = parsed_df.select(
    "customer_id",
    size(col("tags")).alias("num_tags"),
    size(col("browsing_history")).alias("num_browsing_history")
)
display(customer_counts_df)

# 4️⃣ Explodir tags para ver todas as tags únicas
exploded_tags_df = parsed_df.select(
    "customer_id",
    explode(col("tags")).alias("tag")
)
display(exploded_tags_df.distinct())

# 5️⃣ Explodir browsing_history para analisar categorias
exploded_browsing_df = parsed_df.select(
    explode(col("browsing_history")).alias("history")
)

# Contar categorias mais comuns
category_counts_df = exploded_browsing_df.groupBy(col("history.category").alias("category")) \
...
# 1️⃣ Explodir o array recent_orders
exploded_orders_df = parsed_df.select(
    "customer_id",
    explode(col("recent_orders")).alias("order")
)

# Se quiseres, podes mostrar os campos do order
# display(exploded_orders_df.select("customer_id", "order.order_id", "order.amount"))

# 2️⃣ Calcular receita total por cliente
total_revenue_df = exploded_orders_df.groupBy("customer_id") \
                                     .agg(spark_sum(col("order.amount")).alias("total_revenue")) \
...
from pyspark.sql.functions import col, explode, sum, count, desc

#Explodir o array recent_orders
...
#Calcular receita total e número de pedidos por cliente
customer_revenue_df = orders_df.groupBy(
    "customer_id",
    "name"
).agg(
    sum("order.amount").alias("total_revenue"),   # corrigido para order.amount

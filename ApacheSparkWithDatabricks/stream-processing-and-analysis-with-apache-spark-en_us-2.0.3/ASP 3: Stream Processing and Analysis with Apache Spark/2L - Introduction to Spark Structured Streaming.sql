...
-- MAGIC
-- MAGIC # 1. Create a schema for the status updates with these fields:
-- MAGIC #  - order_id (LongType)
-- MAGIC # - order_status (StringType)
-- MAGIC # - status_timestamp (LongType)
-- MAGIC # 2. Create a streaming DataFrame that reads JSON files from the path:
-- MAGIC #   /Volumes/dbacademy_retail/v01/retail-pipeline/status/stream_json
-- MAGIC # 3. Set maxFilesPerTrigger to 1
-- MAGIC # 4. Verify that you have created a streaming DataFrame
-- MAGIC
...
-- MAGIC %python
-- MAGIC # 1. Create a triggered streaming query that:
-- MAGIC #   - Processes data from the status_stream every 15 seconds
-- MAGIC #   - Adds a processing_time timestamp column
-- MAGIC #   - Writes to a memory sink named "triggered_status_updates"
-- MAGIC # 2. Run a SQL query to see the processing batches
-- MAGIC
-- MAGIC # Stop any existing queries with the same name
-- MAGIC for q in spark.streams.active:
